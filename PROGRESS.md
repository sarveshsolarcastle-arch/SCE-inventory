...
| Delivery entry (to store or direct to site) | ✅ Done (Phase 5); **Excel paste added 2026-09-21** — the "ruled out with the user" line in REDESIGN-PLAN.md was reversed at their request. Not a reuse of the Phase 4 parser: a delivery line is packs-or-loose, not a quantity |
...
src/
  ...
  components/
    ...
    DeliveryForm.tsx        client form: 3-row goods-received grid, store or direct-to-site,
                            per-row defective count, Excel paste box, and the same
                            fuzzy item matching as the dispatch form (2026-09-21)
  ...
  lib/
    ...
    paste.ts                PURE splitting shared by both paste parsers — TSV lines/cells,
                            "which cell is a number"
    dispatchPaste.ts        PURE paste parser — Excel TSV → {name, quantity} rows,
                            header-row skipping
    dispatchPaste.test.ts   7 unit tests
    deliveryPaste.ts        PURE paste parser — Excel TSV → {name, packs, loose}. A plain
                            number is loose; "2 x 400" is two sealed packs of 400. Also
                            orientPack, which un-reverses "400 x 2" using the item's
                            known pack sizes
    deliveryPaste.test.ts   13 unit tests
...
- ✅ **Account role changes appeared not to save** (2026-09-09). `setUserRole` in [users.ts](src/lib/actions/users.ts) always wrote the new role correctly, but the `<select>` in [users/page.tsx](src/app/users/page.tsx) never re-synced its `defaultValue` after the server action's re-render — React does not re-sync an uncontrolled `<select>`'s chosen `<option>` on rerender, so the dropdown silently reverted to whatever role the page first loaded with, even though the database and the "N active admins" counter above it were both correct. Fixed with `key={u.role}` on the `Select`, forcing it to remount whenever the saved role changes.
- ✅ **A negative minimum-stock value crashed the item form with no message** (2026-09-09). `readItemFields` in [items.ts](src/lib/actions/items.ts) already rejected a negative or non-integer `minStock`, but nothing caught the thrown error, so it reached Next's unhandled-error page instead of the form — a "crash" with no indication of what was wrong. Fixed two ways: `min={0}` on the number inputs in both [items/new/page.tsx](src/app/items/new/page.tsx) and [items/[id]/page.tsx](src/app/items/[id]/page.tsx) stops the browser submitting a negative value at all, and `createItem`/`updateItem` now catch the validation error and redirect back to the same form with `?error=...`, rendered as an inline `Alert` — so a bypass of the browser check (or a non-JS client) still fails softly instead of crashing.
...
- ✅ **A `/ledger` page and pagination, built 2026-09-09.** Two problems, one real: `adjustStock` ([ops/corrections.ts](src/lib/approvals/ops/corrections.ts)) writes a mandatory `reason` and a generated `note` — *"Counted 95 m against a ledger of 100 (-5)..."* — that nothing in the app ever rendered. Adjustments only ever surfaced in the dashboard's ten-row Recent Activity or one item's 50-row history, and cannot appear on a site page at all (an adjustment has no `siteId`) — the app forced a justification for a stock count and then discarded it. `/ledger` fixes that: every `Transaction`, filterable by type (with reason and note as their own columns), reachable from the nav, from a "View all activity →" link on item and site pages, and from the dashboard. The other half of the plan — purging old records to save Turso quota — was investigated and **dropped**: the whole production database is 344 KiB and would take roughly 1,000 years to fill the free tier's 5 GB, while `materialsAtSite`/`materialAcrossSites` ([stock.ts](src/lib/stock.ts)) replay the **entire, unbounded** ledger to derive what a site holds, so purging anything would have silently emptied every site balance older than the cutoff — an invisible failure in the app's core feature. Not re-proposed; see the plan below for the measurements. Alongside the ledger, `/dispatches` and `/deliveries` — previously unbounded `findMany` calls — and the ledger itself are now paginated 50/page via new [pagination.ts](src/lib/pagination.ts) (`parsePage`/`pageArgs`/`pageCount`/`describeRange`/`clampPage`, 15 tests) and [Pager.tsx](src/components/ui/Pager.tsx); `/sites/[id]` and `/items/[id]` were deliberately **not** paginated, since the site page groups transactions by `dispatchId` client-side and a paginated slice could split one dispatch across two pages. `clampPage` exists because a requested page past the end (`?page=99999`) must fall back to the actual last page rather than run a query that comes back empty and render a table with a misleading "no results" message — caught in browser verification, not in the type-checker. ⚠️ **Flagged, not fixed**: this app runs on Vercel's Hobby plan, which prohibits commercial use regardless of whether the site sells anything — a live terms violation for a company's internal system, and a more plausible way for this to "run dry" than record volume ever was.
- ✅ **Transfer challans, a site material statement, and a shared "remaining" figure in the site panel — built 2026-09-09.** Three related gaps. First: a site-to-site transfer wrote one bare `TRANSFER` `Transaction` per item with no grouping record, so it had no challan — `/ledger?type=TRANSFER` rows were the only ones with no document link, because there was no document. Fixed with a new **`Transfer`** model (mirrors `Dispatch`), its own `SCE/TC/…` series in [challan.ts](src/lib/challan.ts) (`TRANSFER_CHALLAN_SEQUENCE_KEY`, folded into `CHALLAN_SEQUENCE_KEYS` so a reset or reseed can't leave it un-rewound), and `transferBatch` in [siteLifecycle.ts](src/lib/actions/siteLifecycle.ts) replacing the old single-item `transferBetweenSites` — a whole site visit now moves several items in one action, one challan, all-or-nothing inside one `prisma.$transaction` (**verified**: a line exceeding stock refuses the whole batch, including the challan number allocation — nothing partially commits). New pages `/transfers`, `/transfers/[id]`, `/transfers/[id]/challan`. Second: no collective statement existed for a customer — `/sites/[id]/challan` now prints everything `materialsAtSite()` currently shows for a site, on one sheet, titled "Material Statement" (a new optional `title` prop on `ChallanSheet`, since labelling a snapshot a "Delivery Challan" would misrepresent it) and numbered `SCE/MS/<yyyy-mm-dd>` — a **deterministic reference, not a Sequence counter**, because this is a live balance re-printable at will, and burning a series number on every print would imply each one is a distinct consignment. Provenance is absent by construction: the lines this page builds carry no `dispatchId`/`deliveryId`/`fromSiteId`, and each line's `note` is explicitly `null`. Third, a real UX bug confirmed by reading the code: `SiteMaterialPanel`'s consume and transfer inputs each capped independently at the raw held quantity, so typing the full amount into one left the other still offering it — and `runTransfer`'s success path never cleared `consumed`, so a completed transfer left stale typed consumption behind. Batching transfers is what made the fix possible: consume and transfer are now the same shape (a typed quantity per row, filed together), so one derived `remaining = row.quantity - consumedQty - transferQty` governs both inputs' `max` and disables both submit buttons whenever any row is over-drawn — **verified** live: typing the full 220 pcs into consume dropped the transfer input's `max` to 0 in the same render, and both submit buttons disabled the instant a combined over-draw was typed. **The backfill is the one place this kind of migration has already broken once** — see commit `78a5454`, which numbered every backfilled row `1` because `dev.db` held exactly one row and the bug never showed. This one was **tested against a scratch database seeded with three out-of-order `TRANSFER` rows before being trusted**: numbers came back 1/2/3, gap-free, ordered by `createdAt` (not insertion order), each `Transaction.transferId` pointing at its own new `Transfer`, and `Sequence` seeded to 3. `challan.test.ts` gained a check that all three challan formats stay mutually distinct for the same `n`.
- ✅ **Four bugs from a review of the above, fixed 2026-09-10.** **HIGH:** `/ledger?from=<garbage>` 500'd — `new Date()` on an unvalidated `?from=`/`?to=` reached Prisma as an Invalid Date, which throws rather than matching nothing. Fixed with new [dateFilter.ts](src/lib/dateFilter.ts) (`parseDateFilter`, 6 tests) — regex-shape-checked, then round-tripped through its own Y/M/D to reject a shape-valid but nonexistent calendar date like `2026-02-30`, which `new Date()` silently rolls over into a real one instead of producing NaN. **MEDIUM:** the Material Statement still read as a delivery challan — its title changed but `ChallanSheet`'s party-block heading ("Delivery Challan For") and empty Received By/Delivered By signature blocks were hardcoded, so a customer could reasonably sign it as one. `ChallanSheet` gained `partyHeading` ("Prepared For" for the statement) and `showSignatures={false}` props. **MEDIUM:** `SI-TAGGED-ITEMS.md` listed four rows under the pre-import generic SKUs `BOX-ACDB`/`BOX-DCDB`, which don't exist in the live catalogue — corrected to `BOX-DCDB-1IN1`/`BOX-DCDB-2IN2`/`BOX-ACDB-2IN2` (confirmed against production); the fourth, `BOX-ACDB-1IN1`, is flagged unconfirmed rather than guessed. **LOW:** the statement's reference used `toISOString()` (UTC) while its printed Date field used `toLocaleDateString` (server-local) — the two could name different calendar days for part of every evening IST. Both now built from the same local Y/M/D. Two latent risks also closed: the transfer backfill aborted the whole migration on any legacy `TRANSFER` row with a NULL `fromSiteId` (schema-optional on `Transaction`, NOT NULL on `Transfer`) — **reproduced and fixed** with a `WHERE fromSiteId IS NOT NULL AND siteId IS NOT NULL` guard on both the insert and the linking update, re-verified against a scratch database seeded with one such row (the two good rows still number correctly; the malformed one keeps `transferId: null`, its current valid state, rather than aborting everyone else's backfill); and `/transfers`/`/transfers/[id]` were missing the direct `requireCapability("ledger:view")` check every other new route has — added. `SiteBlockerCounts` also gained a `transfers` field so a site-delete refusal names "N transfer challans" instead of folding them silently into "N stock movements" — **verified** live: `"…cannot be deleted — it still has 1 stock movement, 1 transfer challan attached."` Not built here: a batch `reverseTransfer` — deferred at the time, **now built, see the entry directly below**; the two pending production migrations were **not** applied (dev-only fix, left for a separate decision); the reviewer's case-sensitivity claim on ledger/item search doesn't hold here — SQLite's `LIKE` (what Prisma's `contains` compiles to) is case-insensitive for ASCII by default, verified live (`?q=cable` matches "Cable Ties"). Flagged for a follow-up, not fixed here: the same missing-`requireCapability` gap pre-exists on `/dispatches` and `/deliveries`.
- ✅ **A batch `stock.reverseTransfer`, built 2026-09-10.** The real gap flagged above: a `TRANSFER` never touches packs (see the `Transfer`/`TransactionType.TRANSFER` schema comments), so `findObstaclesFor` in [ops/corrections.ts](src/lib/approvals/ops/corrections.ts) always refused it with `no_plan` — reversal was not merely un-batched, it was **impossible**, one line or eight. `/transfers/[id]` rendered no reversal control at all. Fixed with a genuinely different obstacle check rather than reusing the pack-based one: [corrections.ts](src/lib/corrections.ts) gained `TransferObstacle`/`describeTransferObstacle`, and `findTransferObstaclesFor` in ops/corrections.ts checks the one thing that actually matters here — whether the destination site's current balance for that item (`itemQuantityAtSite`) still covers what the transfer brought in, i.e. nothing has been consumed or transferred onward from there since. New `stock.reverseTransfer` `OperationKind` (capability `stock:reverse`, same as `reverseDispatch`), `reverseTransfer` in ops/corrections.ts loops every not-yet-reversed `TRANSFER` line of a `Transfer` inside one transaction — all-or-nothing, same shape as `reverseDispatch` — writing one `REVERSAL` row per line (excluded from `materialsAtSite` via the existing `reversedAt: null` filter; nothing to replay backwards since nothing was applied to packs) and reconciling both sites' pickup flags. Wired through the full registry (`summary.ts`, `registry.ts` precheck/execute/revalidate, `args.ts` parser, `actions/corrections.ts`) exactly like every other approval-requestable operation, and a `ReverseButton` on `/transfers/[id]` mirroring `/dispatches/[id]`'s. The point of building it as a batch rather than adding single-line transfer reversal first: a wrong multi-item transfer now takes **one** reason and **one** approval to undo, not one per line.
- ✅ **Seven bugs from a review of the reverseTransfer/challan work, fixed 2026-09-10.** **HIGH:** [transfers/[id]/challan/page.tsx](src/app/transfers/[id]/challan/page.tsx) printed `party = fromSite.customerName` under "Delivery Challan For" against `shipTo = toSite` — a Dispatch/Delivery/Statement always pairs the SAME site's customer with that site itself, but a transfer's two sites can belong to two different customers, so the sheet named a customer the material was never going to (verified live on the seed data: "Delivery Challan For: Sunview Realty" shipped to "Powai Warehouse", which belongs to Hiranandani Estates). Fixed by dropping the customer framing entirely for this one document: `ChallanSheet` gained a `shipToHeading` prop (alongside the existing `partyHeading`), and the transfer challan now prints "Transfer From" / "Transfer To" with both `party` and `shipTo` as the sites themselves. **MEDIUM:** `SI-TAGGED-ITEMS.md` flagged `BOX-ACDB-1IN1` "unconfirmed" from a previous session's truncated query — re-queried production directly (read-only, against the live Turso database) and it exists (`ACDB 1IN1`, stock 1), exactly as the table said; warning removed. **MEDIUM:** `scripts/seed-demo.ts` wrote its one `TRANSFER` row bare (`transferId: null`) — the exact orphan state the transfer backfill's NULL guard exists to tolerate in *legacy* data, freshly manufactured on every seed since. Now creates a real `Transfer` via `nextChallanNo(tx, TRANSFER_CHALLAN_SEQUENCE_KEY)` first, matching `transferBatch`; the closing summary print-out gained a transfers line and count. Saved as a memory — this is the second time a new document model shipped without seed-demo being updated alongside it (the first was the site-delivery challan). **MEDIUM:** the Material Statement's details strip still printed "Challan No." (on a document whose own header comment says outright "it is not one") and a blank ruled "Delivery time" inviting someone to write one down for a live snapshot. `ChallanSheet` gained `documentNoLabel` ("Reference No." for the statement) and `showDeliveryDetails={false}` (hides "Delivery time" and "Their ref."), closing the gap the `partyHeading`/`showSignatures` fix reached everywhere else on the sheet except this one region. **LOW:** `DeleteSiteButton`'s hint hardcoded "no movements, dispatches or deliveries" — three of the six kinds `siteBlockers.ts`'s own `BLOCKERS` list actually checks, `transfers` among the missing three, so the refusal could name "1 transfer challan attached" while the hint claimed transfers weren't a factor. `siteBlockers.ts` gained `BLOCKER_KIND_NAMES`/`ATTACHMENT_KINDS_SUMMARY`, derived from `BLOCKERS` rather than retyped, so the two can't drift apart again. **LOW:** `SiteMaterialPanel` never collected `deliveredBy`/`receivedBy` for a transfer, even though `transferBatch` accepts them, `Transfer` stores them, and `ChallanSheet` prints them — every transfer challan's signature blocks printed blank by construction. Added the same two optional inputs `DispatchBatchForm` already has, next to the destination `<select>`. **LOW:** `/deliveries`, `/deliveries/[id]`, `/dispatches`, `/dispatches/[id]`, `/sites`, `/sites/[id]`, `/items`, `/items/[id]` and `/at-sites` were still missing the direct `requireCapability("ledger:view")` check that `/ledger`, `/transfers` and the challan routes all have — added to all nine, closing the specific follow-up flagged two entries above. Harmless today (every role holds `ledger:view`), but it removes the one convention split left in the app between "pages that check for themselves" and "pages that trust proxy.ts alone". All fixes **verified live**: the reseeded transfer challan now reads "TRANSFER FROM: Andheri Rooftop" / "TRANSFER TO: Powai Warehouse"; the statement prints "Reference No. SCE/MS/…" with no Challan No./Delivery time/Their ref.; a fresh transfer recorded with names "Ramesh"/"Suresh" printed them under Delivered By/Received By; the delete-site hint now reads all six kinds; and `npm test` (199 tests), `tsc --noEmit` and `eslint` all stayed clean throughout.
- ✅ **One more from that same review, fixed 2026-09-11: approval summaries miscounted lines and never pluralised.** `dispatchLabel`/`transferLabel` in [registry.ts](src/lib/approvals/registry.ts) built their `(N lines)` parenthetical from an **unfiltered** `_count: { select: { transactions: true } }` — which also counts the `REVERSAL` rows a reversal itself writes under the same `dispatchId`/`transferId`, so a transfer or dispatch reversed once and then requested again (which happens: a partial reversal, or someone re-raising after a FAILED row) doubled its own count. **Proven** directly against dev data: TC/0002 (one `TRANSFER` line, already reversed) reported `_count.transactions` of 2 unfiltered vs 1 filtered to `type: "TRANSFER"`/`"ISSUE"` — the list pages already get this right (`where: { type: "TRANSFER" }` / `type: "ISSUE"`), so the fix makes the summary match them. Second bug, same lines: no pluralisation — `(1 lines)`, which the codebase explicitly rejects elsewhere (`siteBlockers.ts`'s spelled-out plurals, its `"one of something is singular"` test). New `lineCount()` helper (`"1 line"` / `"3 lines"`) used by both labels. **Verified** by calling the real `OPERATIONS["stock.reverseTransfer"].summarise` / `"stock.reverseDispatch"].summarise` against dev data directly (bypassing the UI, which was fighting a stale frozen duplicate request from testing the day before): TC/0001 (never reversed) → `"(1 line)"`; TC/0002 (reversed) → `"(1 line)"`, not `"(2 lines)"`; the 4-line `HE/GRN/0774` dispatch → `"(4 lines)"`, unchanged. `npm test`/`tsc --noEmit` clean.
- ✅ **Excel paste on Record a Delivery, built 2026-09-21** — reversing "ruled out with the user; deliveries stay typed", at the user's request. REDESIGN-PLAN.md predicted "wiring it in later is small if supplier challans start arriving as spreadsheets"; that held only halfway. The *splitting* was genuinely reusable and moved into new [paste.ts](src/lib/paste.ts), shared with the dispatch parser. The *parsing* was not: a dispatch line is a quantity, but a delivery line is **packs or loose**, and the two land differently — sealed packs go to `PackStock`, loose becomes an `OpenPack` and can be scrapped on arrival at the threshold. So [deliveryPaste.ts](src/lib/deliveryPaste.ts) is its own parser: a plain number is loose, `2 x 400` is two sealed packs of 400, and **nothing guesses that a bare `400` was a roll**. Two decisions worth keeping. First, `400 x 2` on a challan means 2 rolls of 400, not 400 rolls of 2, and the words do not say which — so `orientPack` asks the *item*: when the second number is not a pack size that item has ever had and the first one is, they are the wrong way round; with no known sizes to go on, what was pasted stands. Second, an item with no `packUnit` has **disabled** pack inputs, so a pasted pack on one would have been invisible on screen and still reached the server — `fitPackToItem` folds it into loose, where it can be seen (the same make-the-wrong-state-underivable reasoning as `databaseUrl.ts` and `effectiveFlagged`). The rows also gained the dispatch form's fuzzy matching, and an unmatched/ambiguous/unconfirmed row now **blocks the submit** instead of being sent for the server to reject. 13 new tests (210 total). **Verified live** against `dev.db`, not just typechecked: a 6-line paste skipped the header, oriented a reversed `100 x 3` to 3 × 100, flagged the unmatched row, and recorded — wire `CBL-DC-RED-4` 2625 → 3625 with sealed 500 m rolls 5 → 7, cable ties `TIE-CBL-NYL` 415 → 715 with packets 3 → 6, `CBL-ERT-6` 305 → 505 loose, and `END-CLM-35` (unpackaged, pasted `2 x 50`) 135 → 235. Console and server logs clean throughout.
- ✅ **`next build` no longer queries the production database, fixed 2026-09-21.** The deploy of the paste box failed on Vercel — *"Failed to build /deliveries/new (attempt 1 of 3) because it took more than 60 seconds"*, same for `/dispatches/new`, then `Export encountered an error, exiting the build`. Those were exactly the two pages that commit invalidated in the **restored build cache**; every other page was served from cache and so never re-rendered, which is the only reason this had not fired before. `AppShell` calls `auth()`, so all 28 routes were already dynamic at runtime — but being dynamic does not stop `next build` starting a render pass, and a page's `await prisma...` is fired off before the layout's session read aborts it. Locally SQLite answers instantly and the discarded result is never noticed; on Vercel it goes to Turso in **Mumbai from a US build machine** and hangs. **24 of the 28 pages read the database this way**, so the next commit touching anything shared — a `ui/` button, `tones.ts` — would have invalidated all 24 at once and failed the deploy outright. Fixed with `export const dynamic = "force-dynamic"` on the **root** [layout.tsx](src/app/layout.tsx) so every segment inherits it, rather than per-page or per-query with `connection()`: a fix applied page by page leaves the next page to be written carrying the same landmine. **Measured**, not assumed — with `DATABASE_URL` pointed at an empty scratch file so an opened connection leaves an observable 0-byte file: before, *"Generating static pages (0/28)"* and the file appeared; after, *"(0/3)"* and no file, no connection. Both `--turbopack` and `--webpack` builds stay clean.
- ⚠️ **A stale session cookie locks the whole app, and nothing catches it — DIAGNOSED 2026-09-21, NOT FIXED.** This is the most valuable outstanding item; it is pre-existing, not introduced by the two entries above. [proxy.ts](src/proxy.ts) sets `const isLoggedIn = !!req.auth` — the **JWT signature only**, never the database, and deliberately so (the comment is explicit: permissions.ts pulls in Prisma, which cannot run in the proxy). So a correctly-signed cookie naming a user row that no longer exists reads as logged in, and two things follow. First, every page is let through, then `currentUser()` ([permissions.ts:89](src/lib/permissions.ts)) re-reads the row, finds nothing, and `requireCapability` **throws** — and there is **no `error.tsx` or `global-error.tsx` anywhere in `src/app`**, so with streaming SSR the throw leaves the browser holding an unfinished response: a blank page that buffers forever with no error, and nothing in the Vercel logs. 18 pages call `requireCapability` directly. Second, and worse: `if (isLoggedIn && isLoginPage) redirect("/dashboard")` means **`/login` is unreachable**, so the user cannot sign in again to clear it — a complete lockout with no in-app exit. **Reproduced**, not theorised: temporary instrumentation in `currentUser()` logged `null: db lookup id= cmttsrdzs0000xkvnvqw153yn found= false` against a healthy `ok cmtufaf110000nwvnulwynzjx ADMIN` in the same server — two clients, one holding a session issued against a *different database*, which is the `.env.local` hazard in §10 biting from the session side. Workaround is clearing site cookies. The fix, when someone picks this up: an `error.tsx`/`global-error.tsx` pair so a server throw is never a blank hang again, plus a dead session logging itself out — a route handler that clears the cookie and lands on `/login` (a **route handler**, because only it can set cookies; a server component cannot), with the proxy letting that one path through so it cannot loop.
